...
function statement_cost(ex::Expr, line::Int, src::CodeInfo, mod::Module, params::InferenceParams)
    ...
    elseif head == :gotoifnot
        target = ex.args[2]
        return plus_saturate(target < line ? 40 : 5, argcost) # loops are more expensive than simple branches
    end
    return argcost
end
...
function inline_worthy(body::Array{Any,1}, src::CodeInfo, mod::Module,
    ...
    for line = 1:length(body)
        ...
        elseif stmt isa GotoNode
            thiscost = stmt.label < line ? 40 : 5 # loops are more expensive than simple branches
        else
            continue
        end
